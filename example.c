...
// https://youtu.be/LmQKZmQh1ZQ MENIT 1:18:18 
...
void olivec_draw_line(uint32_t *pixels, size_t pixels_width, size_t pixels_height, int x1, int y1, int x2, int y2, uint32_t color){
  ...
  if (dx != 0){
    //int k  = dy/dx;
    //int c  = y1 - k*x1; garis menjadi horizontal 
    int c  = y1 - dy*x1/dx;// garis menjadi diagonal 
  ...
  for (int y = 0; y < ROWS; ++y) {
    for (int x = 0; x < COLS; ++x) {
      ...
      */
      // t = x/(COLS*2) + y/(ROWS*2); 
      // lerp(a,b,t) => a + (b - a)*t  
      // (radius/2) + (radius/2)*x/(COLS*2) + (radius/2)*y/(ROWS*2); 
      //(radius/2)*(1 + x/(COLS/2) + y/(ROWS*2));
      //radius = lerp(radius/2, radius, t)
